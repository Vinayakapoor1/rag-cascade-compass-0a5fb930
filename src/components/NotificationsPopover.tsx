...
export function NotificationsPopover() {
  ...
  const getIcon = (title: string) => {
    if (title.includes('Compliance') || title.includes('📋')) return <AlertTriangle className="h-4 w-4 text-amber-500 shrink-0" />;
    if (title.includes('Reminder') || title.includes('🔔')) return <Clock className="h-4 w-4 text-blue-500 shrink-0" />;
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="icon" className="relative bg-transparent hover:bg-muted data-[state=open]:bg-muted">
          ...
          {unreadCount > 0 && (
            <span className="absolute -top-0.5 -right-0.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-destructive px-1 text-[10px] font-bold text-destructive-foreground animate-in zoom-in">
